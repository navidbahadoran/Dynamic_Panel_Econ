...
\begin{abstract}
We study dynamic panel models in which the coefficients on lagged outcomes and observed covariates vary across units and time. Every autoregressive coefficient matrix, every slope coefficient matrix, and the interactive-effects matrix has its own fixed low rank; the matrices need not share ranks, factors, or loadings. Rank selection uses one joint rank-at-most-cap+1 pilot to estimate block singular spectra, normalizes those spectra in fitted-value units, and selects each block rank by a deterministic ridge singular-value ratio. The selected rank vector is then refitted once by unpenalized joint fixed-rank least squares. Inference is based on this full-panel post-refit rather than on the spectral pilot or repeated data splitting. For each target, a Riesz representer solves a local Riesz equation and produces observation-level Riesz weights. Individual coefficients and suitable fixed-time means and contrasts use the full-panel plug-in estimator. A fixed two-way split-panel correction removes the leading loading and factor estimation biases for averages taken over many units and many periods. The probability theory is conditional on common shocks, allowing pervasive common shocks while requiring the remaining unit-specific innovations to be conditionally mixing and the predetermined covariates to be conditionally near-epoch dependent. The theory allows dynamic feedback through lagged outcomes, spatial and cross-sectional dependence, conditional heteroskedasticity, separate ranks, and different growth rates for the two panel dimensions. A spatial HAC estimator consistently estimates the conditional variance when cross-sectional dependence is allowed. After rank selection, inference reuses the selected full-panel post-refit for local targets and adds four fixed-rank split-panel fits for two-way averages, with no random folds or implemented buffers.
\end{abstract}
...
Dynamic panel studies often ask whether persistence or covariate effects differ across units, across time periods, or between economically meaningful groups. A common-coefficient model cannot answer these questions, while unrestricted unit--time coefficients cannot be estimated from a single panel. This paper uses separate low-rank restrictions to make the full coefficient matrices estimable without imposing known groups, smooth time paths, or a common latent structure across regressors.

The model contains a separate coefficient matrix for every lagged outcome and observed covariate. An additional low-rank interactive-effects matrix absorbs additive and common conditional-mean heterogeneity. Each matrix has its own rank and latent spaces. This separation is important in applications in which persistence, covariate effects, and common movements need not be generated by the same factors.

Inference is difficult for three reasons. First, all coefficient matrices enter one outcome equation jointly, so population identification must prevent a change in one matrix from being offset by changes in the others. Second, a current disturbance enters future observations through lagged outcomes, while nearby units may have correlated shocks. Third, estimating the loading and factor rows from the same data creates biases of order $T^{-1}$ and $N^{-1}$. These biases are negligible for an individual coefficient but can be first order for an average taken over many units and many periods.

The estimator follows a post-selection architecture. One joint cap+1 spectral pilot estimates the singular spectrum of every coefficient block. Fitted-value normalization and deterministic ridge ratios select the block ranks separately, after which one unpenalized joint fixed-rank least-squares fit is computed at the selected rank vector. Thus, the reported coefficient estimates and standard errors do not inherit the pilot's extra rank or any nuclear-norm shrinkage. A nuclear path may be used only to initialize the two unpenalized constrained fits. For a supplied target, a Riesz representer solves the least-squares Gram equation on the estimated fixed-rank tangent space. This inverse-Gram adjustment translates estimation errors in all autoregressive, slope, and interactive-effect matrices into first-order error for the selected parameter. At an exact full-panel least-squares solution, the residual-weighted Riesz sum is zero, so the Riesz weights are not added as a separate point-estimate correction. They are instead used to derive the influence expansion and the standard error.

For individual coefficients, fixed unit--time lag sums, and suitable fixed-time means and contrasts, the $N^{-1}+T^{-1}$ estimation bias is smaller than the standard error. These targets use the full-panel plug-in estimate. For averages whose weights are spread over many units and many periods, the standard error can be of order $(NT)^{-1/2}$, so the same bias is not negligible. A fixed two-way split-panel correction removes the leading $T^{-1}$ loading bias and $N^{-1}$ factor bias. The correction uses two time-split fits and two unit-split fits, with the full-panel selected ranks held fixed.

The stochastic assumptions are stated conditionally on common shocks. This does not require removing all common shocks. Common shocks may be strong, persistent, and relevant for covariates, conditional variances, and spatial dependence. Common effects in the conditional mean are represented by the coefficient matrices and the interactive-effects matrix; conditional on the common shocks, the remaining unit-specific innovations are weakly dependent over space and time. Predetermined covariates are approximable by recent innovations from nearby units, while dynamic stability controls the effect of current disturbances on distant future outcomes.

The main contribution is a joint inferential framework for several separately ranked coefficient matrices in a dynamic panel. The method combines four ideas that are familiar separately in low-rank analysis but require new arguments in this setting: a joint spectral pilot and blockwise ridge-ratio rank selector; unpenalized joint least-squares refinement after selection; explicit alignment and expansion of the estimated singular spaces; and a target-specific Riesz equation that adjusts for estimation spillovers across all coefficient matrices. The resulting theory provides full-panel inference without random folds for individual coefficients and suitable fixed-time means and contrasts, a fixed split-panel correction for broad two-way averages, and conditional spatial inference under common shocks. The present results assume exact fixed ranks and consistent selection of the separate rank vector; approximate-low-rank inference, growing-rank asymptotics, semiparametric efficiency, and inference robust to rank overestimation are not claimed.

The remainder is organized as follows. \Cref{sec:literature} relates the paper to the existing literature. \Cref{sec:model} presents the model and parameters of interest. \Cref{sec:estimation} defines estimation, rank selection, and variance estimation. \Cref{sec:assumptions} states the maintained assumptions. \Cref{sec:theory} gives the main asymptotic results. \Cref{sec:implementation} describes computation. \Cref{sec:MC} and \Cref{sec:application} contain the Monte Carlo and empirical designs, with numerical output placeholders in this draft. The appendix is organized in theorem order.

%================================================
\section{Related Literature}
\label{sec:literature}

The paper relates to interactive-effects panels, dynamic panels, heterogeneous coefficients, low-rank estimation, and inference under spatial and temporal dependence.

Large-panel factor methods and interactive-effects regressions use latent factors to absorb unobserved common heterogeneity \citep{bai2002,bai2003,pesaran2006,moonweidner2015}. We retain a low-rank interactive-effects matrix but allow every autoregressive and slope coefficient matrix to vary over both units and time. Each coefficient matrix has its own rank and latent spaces; no common factor or loading space is imposed across matrices.

The classical dynamic-panel literature uses instruments or transformations to address short panels with common coefficients \citep{andersonhsiao1981,andersonhsiao1982,arellanobond1991}. Here both panel dimensions are large, the autoregressive coefficients are heterogeneous, and least squares relies on conditional sequential exogeneity. In dynamic interactive-effects models, predetermined regressors and heteroskedastic or correlated idiosyncratic errors generate separate incidental-parameter biases \citep{moonweidner2017}. The loading-side and factor-side biases derived below have the same economic sources, but they arise for target functionals of several separately ranked coefficient matrices under conditional spatial dependence. If lagged outcomes remain endogenous after conditioning on current covariates, observed past information, and the common shocks, the least-squares criterion would need to be replaced by an instrumented one.

Random-coefficient, mean-group, and grouped-pattern methods impose different structures on coefficient heterogeneity \citep{swamy1970,pesaransmith1995,bonhommemanresa2015}. Low rank instead represents variation through latent unit and time dimensions. It supports individual coefficients, cross-sectional and time averages, group contrasts, and lag-sum persistence targets without requiring a known group assignment.

The closest low-rank inferential benchmark is \citet{chernozhukov2023}. They study linear functionals of a matrix that is well approximated by a spiked low-rank structure, allow slowly growing rank, initialize estimation by nuclear-norm regularization, and use unpenalized least-squares refinements together with an explicit rotation analysis of the estimated singular spaces. Under their model, the resulting procedure attains a semiparametric efficiency bound. We adopt the same post-regularization principle---a convex nuclear-norm pilot followed by unpenalized refinement---and the same broad emphasis on singular-space alignment, but their estimator does not transfer mechanically to the present model. Here several coefficient matrices enter the same regression through different stochastic regressors, one regressor is the lagged dependent variable, and the unit-specific disturbances may be spatially dependent conditional on common shocks. The fixed-rank refit and the local Gram equation must therefore be joint across the autoregressive, slope, and interactive-effect matrices.

The target-specific Riesz equation addresses the corresponding first-order inferential problem. It represents the derivative of a selected parameter under the joint conditional least-squares Gram form, and therefore accounts simultaneously for estimation errors in every coefficient matrix. This inverse-Gram adjustment is related in purpose to the rotation and least-squares refinements in \citet{chernozhukov2023}, but the present paper establishes validity rather than a semiparametric efficiency result. An efficiency analysis under conditional heteroskedasticity and spatial dependence would require an optimally covariance-weighted version of the joint Gram equation and is outside the scope of this paper.

\citet{choi2024} obtain low-rank completion inference without implemented sample splitting by combining a nuclear-norm pilot with loading and factor least-squares refinements. Their proof uses an auxiliary leave-one-out estimator that is not computed. Our full-panel analysis follows the same broad proof principle, but the auxiliary object must delete a space--time neighborhood rather than a single observation: a current disturbance reappears in future lagged outcomes and may remain correlated with nearby units. For individual coefficients and suitable fixed-time parameters, no split is implemented. The four split-panel fits used for broad two-way averages serve a different purpose: they estimate and remove the leading $T^{-1}$ loading bias and $N^{-1}$ factor bias rather than creating independence.

Recent rank-robust low-rank inference provides a useful contrast. \citet{choi2026rank} construct inference that remains valid when a prespecified rank is no smaller than the true rank and show that overestimating the low-rank space can itself generate a nonnegligible bias. Our current theory instead requires consistent selection of a separate rank for every coefficient matrix because the tangent spaces and joint Riesz equation are rank specific. Extending rank-robust inference to the present model would require controlling the bias from overestimating several interacting latent spaces and its propagation through the dynamic regression. We therefore report rank-selection and neighboring-rank diagnostics but do not describe the resulting intervals as rank robust.

The comparison also clarifies the scope of the maintained low-rank assumptions. \citet{chernozhukov2023} and \citet{choi2024} permit forms of approximate low rank and slowly growing rank in their respective settings. We impose exact fixed ranks because approximation remainders in an autoregressive coefficient matrix can propagate through future outcomes, while approximation errors from all coefficient matrices enter the same local Gram and Riesz equations. The spectral selector and inferential theorems are therefore stated for exact fixed rank. Approximate-low-rank inference and growing-rank theory are left for future work.

Related work on matrix completion and causal panels includes \citet{athey2021matrix} and \citet{bai_ng2021matrix}. Those settings motivate low-rank recovery but do not directly cover a dynamic equation with several stochastic-regressor coefficient matrices, predetermined covariates, and conditional spatial dependence.

Ridge eigenvalue ratios are used by \citet{pu2025} to estimate reduced rank in a spatio-temporal model, and modified eigenvalue-ratio methods also appear in recent tensor-factor work such as \citet{barigozzihe2023}. These papers motivate the spectral-ratio principle, but they study different observed spectral objects and impose model-specific dependence and growth conditions. We do not import their ridge rates or relative-dimension restrictions. Instead, the blockwise result below follows from this paper's joint pilot operator rate, strong coefficient-matrix signals, and rectangular-panel growth condition. For computation, \citet{moonweidner2026} show that nuclear-norm panel estimators can provide effective starting values for subsequent least-squares refinements, and \citet{gaoabsil2022} develop rank-adaptive Riemannian updates. Here a nuclear path is optional warm-start machinery only; it does not define ranks or enter the rank-consistency theorem.

The conditional weak-dependence framework follows the spatial near-epoch-dependence approach of \citet{jenishprucha2012}. Spatial HAC variance estimation follows the logic of \citet{conley1999gmm}. Entrywise low-rank perturbation results motivate the pointwise loading and factor analysis, while the dynamic and dependent setting requires additional control of future lagged outcomes and local spatial dependence.
...
u_{it}.
\end{equation}
Here $y_{i,t-\ell}$ is the $\ell$th lag of the dependent variable, $x_{it,k}$ is the $k$th observed covariate, and $u_{it}$ is the remaining unit-specific innovation. The term $h_{0,it}$ is an unobserved interactive fixed effect that captures latent common conditional-mean variation with heterogeneous unit loadings and is treated as a nuisance parameter. The autoregressive coefficients $a_{0,it,\ell}$, for $\ell=1,\ldots,P$, and the slope coefficients $\beta_{0,it,k}$, for $k=1,\ldots,K$, may vary across both units and time. The initial values $y_{i,0},\ldots,y_{i,1-P}$ are observed. Throughout, $P$ and $K$ are fixed and known.
...
Let
$$
\br=(r_{A,1},\ldots,r_{A,P},r_{B,1},\ldots,r_{B,K},r_H)
$$
denote a rank vector. If $\bar r_{A,\ell}$, $\bar r_{B,k}$, and $\bar r_H$ are researcher-specified rank caps, define
$$
\calR_{\max}
=
\prod_{\ell=1}^{P}\{0,1,\ldots,\bar r_{A,\ell}\}
\times
\prod_{k=1}^{K}\{0,1,\ldots,\bar r_{B,k}\}
\times
\{0,1,\ldots,\bar r_H\}.
$$
This is the reportable rank set. For construction and analysis of one spectral pilot only, define
$$
\calR_{\max}^{+}
:=
\prod_{\ell=1}^{P}\{0,1,\ldots,\bar r_{A,\ell}+1\}
\times
\prod_{k=1}^{K}\{0,1,\ldots,\bar r_{B,k}+1\}
\times
\{0,1,\ldots,\bar r_H+1\}.
$$
The extra pilot rank is not reportable. The number of coefficient matrices and all reporting and pilot caps remain fixed.
...
\right\}.
$$
Every $\bDelta\in\calD_{\max}^{+}$ satisfies
$\rank(\bDelta_M)\le\bar r_M+1+r_{M,0}\le2\bar r_M+1$ block by block.

Write the normalized least-squares loss as
$$
\mathcal L_{NT}(\bTheta)
=
\frac1{2NT}\sum_{i,t}\{y_{it}-\mathcal Y_{it}(\bTheta)\}^2.
$$
Define the cap+1 spectral pilot by
\begin{equation}
\label{eq:rank_cap_pilot}
...
\mathcal L_{NT}(\bTheta).
\end{equation}
An approximate solution is permitted under the objective-gap condition in \Cref{cor:cap_plus_one_pilot}. The truth is feasible. The extra rank exists only to expose one singular value beyond each reporting cap. The pilot estimates block spectra and may initialize later optimization, but it is never the reported coefficient estimator.

To put coefficient-block spectra on comparable fitted-value scales, use
$$
w_{A,\ell}
=
\left(\frac1{NT}\sum_{i,t}y_{i,t-\ell}^2\right)^{1/2},
\qquad
w_{B,k}
=
\left(\frac1{NT}\sum_{i,t}x_{it,k}^2\right)^{1/2},
\qquad
w_H=1.
$$
The reference $w_{A,1}$ is bounded above and away from zero with probability approaching one by the maintained innovation, moment, stability, and prediction-identification conditions. For every coefficient matrix $M$ and $j\ge1$, define
\begin{equation}
\label{eq:rank_normalized_spectrum}
\widehat\lambda_{M,j}
:=
\left(\frac{w_M}{w_{A,1}}\right)^2
\frac{\sigma_j(\widehat\bM^{\mathrm{pil}})^2}{NT}.
\end{equation}
This dimensionless normalization places every block spectrum in common fitted-value units. A coherent fixed rescaling of a covariate multiplies its weight and divides its coefficient matrix by the same magnitude; a coherent outcome-unit change likewise cancels through the coefficient transformation and $w_{A,1}$. Thus such unit changes do not alter \eqref{eq:rank_normalized_spectrum} or the selected ranks. The coefficient box is applied in stored canonical scientific units.
...
a_{NT}:=\frac1{\log(NT)},
\qquad
\widehat\lambda_{M,0}:=1.
\end{equation}
\iffalse
$$
\mathcal J_{NT}^{\mathrm{obsolete}}(\bTheta)
=
\sum_{\ell=1}^{P}w_{A,\ell}\|\bA^{(\ell)}\|_*
+
\sum_{k=1}^{K}w_{B,k}\|\bB^{(k)}\|_*
+
w_H\|\bH\|_*.
$$
For a coefficient matrix $\bM$, let $\nabla_M\mathcal L_{NT}(\bm0)$ be the gradient of the loss with respect to that matrix at the zero coefficient collection. Define
\begin{equation}
\label{eq:rank_lambda_max}
\lambda_{\max}
=
\max_{M}
\frac{\|\nabla_M\mathcal L_{NT}(\bm0)\|_{\op}}{w_M}.
\end{equation}
The zero coefficient collection satisfies the penalized first-order conditions for every $\lambda\ge\lambda_{\max}$. Thus, \eqref{eq:rank_lambda_max} supplies a reproducible upper endpoint rather than an arbitrary starting penalty.

Let $\Lambda_{NT}=\{\lambda_1>\cdots>\lambda_L>0\}$ be a finite decreasing screening grid with $\lambda_1=\lambda_{\max}$. The exact geometric grid and its denser sensitivity version are specified in \Cref{sec:implementation}; they are computational choices and do not enter the rank-consistency theorem.

In addition to the convex path, compute one rank-cap pilot
\begin{equation}
\label{eq:rank_cap_pilot}
\widehat\bTheta^{\max}
\in
\argmin_{\substack{\|\bTheta\|_{\max}\le B\\
\rank(\bM)\le\bar r_M\ \text{for every coefficient matrix }\bM}}
\mathcal L_{NT}(\bTheta).
\end{equation}
This is one bounded-rank fit, not a search over $\calR_{\max}$. It may be computed by rank-adaptive fixed-rank updates. In the baseline procedure, $\widehat\bTheta^{\max}$ is the candidate-coverage pilot. More generally, the theory permits any included pilot $\widehat\bTheta^{\mathrm{pil}}$ satisfying the operator-norm condition in \Cref{thm:rank_consistency}.

Let $\tau_{NT}>0$ be a singular-value threshold. For every path estimate and for the rank-cap pilot, record the number of singular values of each coefficient matrix exceeding $\tau_{NT}$. Denote the resulting rank vectors by $\widetilde\br(\lambda)$ and $\widehat\br^{\max}$. The theory restricts only the rate of $\tau_{NT}$; the baseline numerical value and its sensitivity multipliers are specified in \Cref{sec:implementation}.
...
\{\widehat\br^{\max}\}
\cup
\{\widetilde\br(\lambda):\lambda\in\Lambda_{NT}\}.
$$
For each $\br\in\calR_{\mathrm{base}}$, retain $\br$ and every valid one-rank change $\br\pm\be_m$, where $\be_m$ changes the rank of only one coefficient matrix. The candidate set is
\begin{equation}
\label{eq:rank_candidate_set}
\calR_{\mathrm{cand}}
=
\bigcup_{\br\in\calR_{\mathrm{base}}}
\left(
\{\br\}
\cup
\{\br\pm\be_m:m=1,\ldots,P+K+1\}
\right)
\cap\calR_{\max},
\end{equation}
with invalid and duplicate vectors removed. Before duplicates are removed, the procedure generates at most
$$
(|\Lambda_{NT}|+1)\{1+2(P+K+1)\}
$$
candidates. This grows linearly in the number of coefficient matrices rather than exponentially as in a Cartesian search or a full radius-one neighborhood.

For every $\br\in\calR_{\mathrm{cand}}$, compute the unpenalized fixed-rank post-refit $\widehat\bTheta(\br)$ in \eqref{eq:fixed_rank_estimator}. Its residual mean square is
\begin{equation}
\label{eq:rank_residual_mse}
\widehat Q_{NT}(\br)
=
\frac1{NT}\sum_{i,t}
\{y_{it}-\mathcal Y_{it}(\widehat\bTheta(\br))\}^2.
\end{equation}
For a rank-$r$, $N\times T$ matrix, $r(N+T-r)$ is the number of locally free parameters. Hence
$$
\begin{aligned}
d(\br)
={}&
\sum_{\ell=1}^{P}r_{A,\ell}(N+T-r_{A,\ell})
+
\sum_{k=1}^{K}r_{B,k}(N+T-r_{B,k})
+
r_H(N+T-r_H).
\end{aligned}
$$
For a positive deterministic sequence $\kappa_{NT}$, select
\begin{equation}
\label{eq:rank_selector}
\widehat\br
\in
\argmin_{\br\in\calR_{\mathrm{cand}}}
\left[
\log\{\widehat Q_{NT}(\br)\}
+
\kappa_{NT}\frac{d(\br)}{NT}
\right].
\end{equation}
Ties are resolved in favor of the smaller dimension. Before finalizing the selected vector, the implementation evaluates every valid one-coordinate neighbor of the provisional minimizer that has not already been post-refitted. If an added vector has a lower information criterion, it is included and the minimization is repeated until no omitted one-coordinate neighbor improves the criterion. This local completion adds at most a small number of fits and prevents a coarse screening path from stopping immediately next to a better rank vector. After selecting $\widehat\br$, all preceding formulas are used with $\br=\widehat\br$ and $\widehat\bTheta=\widehat\bTheta(\widehat\br)$. Rank selection is not repeated for parameters of interest or split panels. Nuclear shrinkage is therefore removed before residual fit is compared and before any inferential object is constructed.

The nuclear path implements the post-regularization idea of using a convex low-rank estimate to locate promising singular spaces and provide warm starts, while the unpenalized post-refits jointly update all autoregressive, slope, and interactive-effect matrices. Rank-adaptive Riemannian updates can be used for the rank-cap pilot and nearby post-refits. These computational ideas are related to \citet{chernozhukov2023,moonweidner2026,gaoabsil2022}, but the rank-consistency result below is model specific. It depends on pilot accuracy, threshold separation, and information-criterion separation, not on a particular nuclear grid. The implementation reports rank-stability diagnostics because the inferential theorems require consistent rank selection and do not provide robustness to rank overestimation.

\fi

For rank zero define
\begin{equation}
\label{eq:rank_zero_ratio}
R_{M,0}:=\frac{\widehat\lambda_{M,1}+a_{NT}}{1+a_{NT}}.
\end{equation}
For $j=1,\ldots,\bar r_M$, define
\begin{equation}
\label{eq:rank_ratio}
R_{M,j}
:=
\frac{\widehat\lambda_{M,j+1}+a_{NT}}
{\widehat\lambda_{M,j}+a_{NT}}.
\end{equation}
At the reporting cap, the numerator uses the genuine $(\bar r_M+1)$st pilot singular value rather than a mechanically truncated zero. Select block ranks separately by
\begin{equation}
\label{eq:rank_selector}
\widehat r_M
:=
\min\argmin_{0\le j\le\bar r_M}R_{M,j},
\qquad
\widehat\br
:=
(\widehat r_{A,1},\ldots,\widehat r_{A,P},
\widehat r_{B,1},\ldots,\widehat r_{B,K},\widehat r_H).
\end{equation}
Exact ties select the smaller rank. Rank zero competes through \eqref{eq:rank_zero_ratio}; no test, significance level, or cutoff is introduced. No common rank or common latent space is imposed.

Finally compute exactly one literal unpenalized constrained fixed-rank post-refit $\widehat\bTheta(\widehat\br)$ from \eqref{eq:fixed_rank_estimator}. Every reported coefficient, tangent space, Riesz representer, target estimate, split correction, residual, standard error, and interval is constructed from this final post-refit. Rank selection occurs once on the full panel and is not repeated on split panels. A nuclear-norm path may optionally initialize the cap+1 pilot or final post-refit, but it does not define candidate ranks, selected ranks, or any condition in the rank-consistency theorem.
...
c\|\bDelta\|^2
\end{equation}
holds both (i) for every nonzero admissible difference $\bDelta=\bTheta-\bTheta_0$, with $\bTheta$ ranging over the ranks in $\calR_{\max}$ and satisfying the coefficient bound, and (ii) for every nonzero tangent direction $\bDelta\in\calT_0$.
\end{assumption}
This is an identification condition on the coefficient matrices, not a bound on $u_{it}$. The first part says that two distinct admissible low-rank coefficient collections cannot produce the same conditional fitted values. The second part is the local version needed for the Riesz equation and the loading--factor normal equations: it makes the conditional least-squares Gram form coercive on $\calT_0$. A simple sufficient setting is one in which the lagged outcomes, covariates, and constant have conditionally nonsingular second moments after their low-rank directions are taken into account. Identification can fail, for example, if a covariate is exactly equal to a lagged outcome and the associated coefficient matrices can offset one another, or if a constant covariate duplicates the unrestricted role of $\bH_0$. The corresponding upper bound follows from the fixed numbers of lags and covariates and the moment bounds in \Cref{a:moments}.

\begin{assumption}[Pilot-only prediction identification]
\label{a:pilot_identification}
For construction of the cap+1 spectral pilot only, part~(i) of \Cref{a:identification} additionally holds on $\calD_{\max}^{+}$: for some fixed $c_+>0$,
$$
\Ezero\!\left[
\sum_{i,t}\mathcal Y_{it}(\bDelta)^2
\middle|\mathscr C
...
\end{assumption}
This is a localized strengthening of the domain of the existing prediction-identification condition. It is used only for the data-driven spectral pilot and may have a smaller curvature constant than the original reporting-class bound. Supplied-rank estimation and inference continue to use \Cref{a:identification} on $\calR_{\max}$; no temporal, spatial, moment, signal, target, or growth assumption is changed.
...
\end{equation}
The quantity $b_{NT}$ is used only in the theoretical rates and reflects the finite $8+\eta$ moments; it is not selected from the data. Rank selection uses the deterministic ridge $a_{NT}=1/\log(NT)$ in \eqref{eq:rank_ridge}; there is no data-selected rank penalty or ridge multiplier.
...
\end{equation}
\end{assumption}
This condition allows $N$ and $T$ to grow at different rates but requires both dimensions to be large enough relative to the moment and spatial-complexity terms. It delivers the recovery rate used by the full-panel estimator and the cap+1 spectral pilot, and it implies $a_{NT}\to0$ and $\zeta_{NT}/a_{NT}\to0$. No aspect-ratio restriction is imposed.

The assumptions above contain the maintained restrictions on the data-generating process, parameters of interest, and panel growth. No separate high-level conditions are imposed. The appendix derives the uniform low-rank score and Gram bounds, the proof-only local-deletion rates, the preservation of ranks and Gram matrices by the balanced random splits, feasible spatial variance estimation, and rank consistency. Short numerical requirements for approximate optimization are stated with the results that use them.
...
\left(\frac1N+\frac1T\right).
$$
The logarithmic power reflects the fixed-dimensional unit--time blocking and the low-rank covering argument in \Cref{lem:weighted_concentration,prop:uniform_concentration}. The panel-growth assumption is stronger than what is needed for $\zeta_{NT}\to0$ and continues to imply all rank-threshold and split-panel rates below.
...
\iffalse
\begin{theorem}[Rank consistency and selected-rank inference]
\label{thm:rank_consistency}
Suppose \Cref{a:stab,a:exog,a:geometry,a:ned,a:moments,a:signal,a:identification,a:gram,a:growth} hold and $\br_0\in\calR_{\max}$. Suppose the candidate set contains the thresholded rank vector of at least one pilot $\widehat\bTheta^{\mathrm{pil}}$ satisfying
\begin{equation}
\label{eq:pilot_operator_rate}
\max_M
\|\widehat\bM^{\mathrm{pil}}-\bM_0\|_{\op}
=
o_p(\tau_{NT}),
\qquad
\tau_{NT}=o(\sqrt{NT}).
\end{equation}
Let the information-criterion penalty satisfy
\begin{equation}
\label{eq:rank_penalty_rates}
\zeta_{NT}
=
o\!\left(
\kappa_{NT}\frac{N+T}{NT}
\right),
\qquad
\kappa_{NT}\frac{N+T}{NT}
\longrightarrow0.
\end{equation}
Assume that the normalized objective errors of the candidate post-refits are
$$
o_p\!\left(
\kappa_{NT}\frac{N+T}{NT}
\right).
$$
Then the screened information criterion \eqref{eq:rank_selector} satisfies
\begin{equation}
\label{eq:rank_consistency}
\Pzero(\widehat\br=\br_0\mid\mathscr C)\longrightarrow_p1.
\end{equation}
Consequently, \Cref{thm:recovery,thm:target_expansion,thm:twoway} remain valid when the supplied ranks are replaced by the selected ranks. Rank selection is performed once on the full panel and is not repeated in the split panels.
\end{theorem}

\begin{remark}[Baseline pilot and threshold]
\label{rem:baseline_rank_pilot}
Under the maintained assumptions, the rank-cap pilot in \eqref{eq:rank_cap_pilot} obeys
$$
\max_M
\|\widehat\bM^{\max}-\bM_0\|_{\op}
=
O_p\{\sqrt{NT\zeta_{NT}}\}
$$
when its normalized objective gap is $o_p(\zeta_{NT})$. It therefore satisfies \eqref{eq:pilot_operator_rate} whenever
$$
\sqrt{NT\zeta_{NT}}=o(\tau_{NT})
\quad\text{and}\quad
\tau_{NT}=o(\sqrt{NT}).
$$
The baseline threshold in \Cref{sec:implementation} has these properties under \Cref{a:growth}. The nuclear path enriches the candidate set and supplies warm starts, but its grid is not needed for candidate coverage.
\end{remark}

\begin{remark}[Strong singular values under approximate low rank]
\label{rem:approx_rank_screening}
The thresholding argument has a limited screening interpretation beyond exact low rank. If, for one coefficient matrix,
$$
\bM_0=\bM_{0,r}+\bR_M,
$$
where $\bM_{0,r}$ has $r$ singular values of order $\sqrt{NT}$ and $\|\bR_M\|_{\op}=o(\tau_{NT})$, then a pilot satisfying \eqref{eq:pilot_operator_rate} identifies the number $r$ of strong singular values by the same perturbation argument. This observation concerns screening only. All inferential theorems in the paper continue to assume exact fixed ranks because the approximation remainder can enter the parameter of interest, the dynamic recursion, and the joint Riesz equation.
\end{remark}

\fi

\begin{theorem}[Blockwise ridge-ratio rank consistency]
\label{thm:rank_consistency}
Suppose \Cref{a:stab,a:exog,a:geometry,a:ned,a:moments,a:signal,a:identification,a:pilot_identification,a:gram,a:growth} hold, $\br_0\in\calR_{\max}$, and a joint spectral pilot satisfies
\begin{equation}
\label{eq:pilot_operator_rate}
\max_M
\|\widehat\bM^{\mathrm{pil}}-\bM_0\|_{\op}
...
O_p\{\sqrt{NT\zeta_{NT}}\}.
\end{equation}
Construct \eqref{eq:rank_normalized_spectrum}--\eqref{eq:rank_selector} with $a_{NT}=1/\log(NT)$. Then
\begin{equation}
\label{eq:rank_consistency}
\Pzero(\widehat\br=\br_0\mid\mathscr C)
\longrightarrow_p1.
\end{equation}
Consequently, \Cref{thm:recovery,thm:target_expansion,thm:twoway} remain valid when supplied ranks are replaced by ridge-ratio selected ranks. Rank selection is performed once on the full panel and is not repeated in the split panels.
\end{theorem}

\begin{corollary}[Cap+1 spectral-pilot rate]
\label{cor:cap_plus_one_pilot}
Under the assumptions of \Cref{thm:rank_consistency}, let the feasible pilot in \eqref{eq:rank_cap_pilot} satisfy
\begin{equation}
\label{eq:pilot_objective_gap}
...
Then
$$
\|\widehat\bTheta^{\mathrm{pil}}-\bTheta_0\|
=O_p\{\sqrt{NT\zeta_{NT}}\},
\qquad
\max_M\|\widehat\bM^{\mathrm{pil}}-\bM_0\|_{\op}
=O_p\{\sqrt{NT\zeta_{NT}}\}.
$$
Observable feasibility, multistart objective agreement, stationarity, and constraint diagnostics do not certify the unknown global infimum in \eqref{eq:pilot_objective_gap}.
\end{corollary}

\begin{remark}[Rectangular ridge separation]
Let $m=NT$ and $G_{NT}=m^{4/(8+\eta)}\{\log m\}^{d_s+6}(1/N+1/T)$. Under \Cref{a:growth},
$$
\frac{\zeta_{NT}}{a_{NT}}
=
\frac{G_{NT}}{m^{2/(8+\eta)}\log m}
\longrightarrow0,
\qquad
a_{NT}\longrightarrow0.
$$
This identity retains $1/N+1/T$ and requires neither $N=T$, $N/T\to c$, nor $N\asymp T$.
\end{remark}
...
The implementation separates screening, post-refitting, rank selection, and inference. This separation is important: the nuclear estimates are used only to propose ranks and initialize numerical optimization, whereas every reported coefficient and standard error is based on an unpenalized fixed-rank post-refit.

A practical implementation proceeds as follows.

\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item \textit{Preprocessing and admissible ranks.} Standardize the lagged outcomes and observed covariates used in the screening problems. Specify the matrix-specific rank caps, the coefficient bound $B$, and the admissible set $\calR_{\max}$. Store the transformations so that all final estimates can be returned to the original units.

\item \textit{Convex screening and the rank-cap pilot.} Compute the penalty weights and $\lambda_{\max}$ in \eqref{eq:rank_lambda_max}. Solve the prespecified warm-started nuclear path described in \Cref{subsec:tuning}. Separately compute the single rank-cap pilot in \eqref{eq:rank_cap_pilot}. The path supplies additional rank proposals and warm starts; the rank-cap pilot supplies the baseline candidate-coverage safeguard.

\item \textit{Candidate generation.} Threshold the singular values of every preliminary estimate, add all valid one-coordinate neighboring ranks, intersect with $\calR_{\max}$, and remove duplicates. Record which pilot generated each candidate and whether the rank-cap pilot rank is retained.

\item \textit{Unpenalized post-refitting and final rank choice.} For every candidate rank vector, compute the joint unpenalized fixed-rank least-squares post-refit. Compare only these post-refits using \eqref{eq:rank_selector}. The penalized objective values and penalized coefficient matrices do not enter the final information criterion. Retain the selected post-refit as $\widehat\bTheta$.

\item \textit{Rank-stability assessment.} Fit any valid one-coordinate neighbor of the selected vector that has not already been evaluated. If one has a lower information criterion, add it to the candidate set and repeat the selection step. Report the final selected rank vector, the rank-cap pilot vector, the distinct nuclear-path rank vectors, the candidate count after duplicate removal and local completion, the smallest and second-smallest information criteria, and whether the selected vector changes under the denser path, threshold multipliers, penalty multipliers, or larger rank caps. For each main parameter, report the change in its plug-in estimate under the one-coordinate neighboring rank vectors with the smallest information criteria. These are diagnostics, not rank-robust confidence intervals.

\item \textit{Parameters and Riesz systems.} For each parameter of interest, compute the plug-in estimate and the empirical Riesz representer from the selected unpenalized post-refit. Report the full-panel plug-in estimate for the parameters covered by \Cref{thm:target_expansion}. For a two-way average covered by \Cref{thm:twoway}, compute the two time-split and two unit-split fits, holding $\widehat\br$ fixed, and form the corrected estimate in \eqref{eq:half_panel_correction}.

\item \textit{Variance estimation and numerical reporting.} Compute the diagonal, spatial, or known-cluster variance estimator appropriate for the maintained cross-sectional dependence. For the corrected estimate, combine the full- and split-panel residual--Riesz products as in \eqref{eq:corrected_spatial_variance}. Store objective values, first-order residuals, coefficient-bound activity, retained singular values, Gram-matrix eigenvalues, Riesz-equation residuals, split-panel group counts, spatial distance cutoffs, runtimes, and random seeds.
\end{enumerate}

The full-panel estimate initializes all four split-panel fits. The unit split is balanced within every group entering the parameter of interest and, when useful, within broad geographic strata. The time split is balanced over the full sample. For a selected time $t$, its observed lagged outcomes remain available as predetermined regressors even when a lagged time belongs to the other partition; the dynamic process is not restarted.

A selected rank at its imposed cap, disagreement between the baseline and denser screening paths, failure of the rank-cap pilot vector to enter the candidate set, a nearly tied information criterion, or a nearly singular Riesz system is reported as rank sensitivity. The paper does not describe such results as rank robust. A primary interval is not reported when the selected rank remains unresolved at its cap, the numerical fit fails the stated tolerances, the coefficient bound is active, a split loses a selected rank or target support, or the relevant Gram or Riesz system is numerically singular.

\subsection{Computation of the rank-constrained estimator}

For a supplied rank vector, write each positive-rank coefficient matrix as $\bM=\bm L_M\bm F_M'$ and set every rank-zero matrix equal to zero. Conditional on the factor rows, the least-squares criterion separates into loading regressions across units. Conditional on the loading rows, it separates into factor regressions across time. The estimator can therefore be computed by warm-started alternating least squares or by a fixed-rank manifold optimization routine. The optimization is joint: every update is evaluated using residuals formed from all autoregressive, slope, and interactive-effect matrices. For the rank-cap pilot, rank-adaptive Riemannian updates may increase a rank when the objective gradient has a substantial direction outside the current tangent space and reduce a rank when a fitted singular value falls below the numerical threshold. After each update, the factors and loadings are renormalized without changing their product.

Each candidate post-refit uses two deterministic starting values. The first is the nuclear-path or rank-cap estimate whose thresholded rank vector has the smallest $\ell_1$ distance from the candidate, truncated or augmented to the candidate ranks. The second is the rank-cap pilot truncated to the candidate ranks. The lower-objective solution is retained. A third randomized start is used only when the two deterministic solutions disagree by more than the reported objective tolerance. A fit is retained only when the objective is stable across the best starts and the first-order residual is below the reported tolerance. Candidate post-refits are independent after their starting values are formed and can be run in parallel. The selected full-panel solution supplies starting values for all split-panel fits. No coefficient estimate from the nuclear path is carried directly into a reported table or standard error.

After estimating the matrices, construct the tangent projection from their singular spaces. The empirical Riesz equation should be solved without forming a dense tangent-space basis. The required linear operator maps a proposed matrix collection $\bQ$ to the fitted-value changes $\{\mathcal Y_{it}(\bQ)\}_{i,t}$ and maps an observation array back through the adjoint fitted-value map followed by the tangent projection. Conjugate gradients or LSQR can then solve the positive-definite system using matrix--vector products. The same operator and preconditioner are reused across parameters of interest; only the direction $\bD$ changes. The weighted residual identity \eqref{eq:weighted_residual_zero} and the residual of the Riesz equation are reported as numerical checks.

For the spatial variance calculation, neighbor lists are precomputed for pairs satisfying $d_N(i,k)\le h_N$. The calculation then has cost proportional to $TNv_N(h_N)$ rather than $TN^2$.

\subsection{Tuning choices}
\label{subsec:tuning}

The rank theorem and the numerical implementation use different levels of specification. Theorem~\ref{thm:rank_consistency} requires an accurate included pilot, a separating singular-value threshold, and an information-criterion penalty satisfying the stated rates. The exact nuclear grid, threshold multipliers, optimization tolerances, and sensitivity calculations below are prespecified numerical choices. They affect screening resolution and computation, not the form of the consistency theorem.

The rank caps should be modest but high enough that the selected ranks do not repeatedly reach their upper bounds. The baseline nuclear path is the geometric grid
\begin{equation}
\label{eq:rank_lambda_grid}
\lambda_m
=
\lambda_{\max}\gamma^{m-1},
\qquad
m=1,\ldots,L,
\qquad
L
...
1+
\left\lceil
\frac{\log(\epsilon)}{\log(\gamma)}
\right\rceil.
\end{equation}
The implementation fixes $\gamma=0.8$ and $\epsilon=0.01$, giving $L=22$ points between $\lambda_{\max}$ and approximately $0.01\lambda_{\max}$. These values determine numerical resolution and warm-start spacing; they are not used in \Cref{thm:rank_consistency}. The denser path with $\gamma_{\mathrm{dense}}=\sqrt{0.8}$ inserts one logarithmic midpoint between adjacent baseline values and is a prespecified sensitivity calculation. The implementation reports $\lambda_{\max}$, $\lambda_L/\lambda_{\max}$, the number of distinct thresholded path ranks, the size of $\calR_{\mathrm{cand}}$, and whether the denser path changes the selected rank or the ordering of the two smallest information criteria.

The penalty weights are the sample root-mean-square scales of the corresponding lagged outcome or covariate, with $w_H=1$. The baseline singular-value threshold is
$$
\tau_{NT}=\frac{\sqrt{NT}}{\log(NT)},
$$
with sensitivity to the constants $1/2$ and $2$ multiplying this threshold. This choice lies above the rank-cap pilot error and below the order-$\sqrt{NT}$ genuine singular values under the maintained rates. The theory permits any threshold satisfying \eqref{eq:pilot_operator_rate}.

The theory-aligned baseline information-criterion penalty is
\begin{equation}
\label{eq:rank_penalty}
...
\log(NT)\,\zeta_{NT},
$$
this sequence satisfies \eqref{eq:rank_penalty_rates} under \Cref{a:growth}. Sensitivity to the constants $1/2$ and $2$ multiplying \eqref{eq:rank_penalty} is reported. The exhaustive Cartesian search is used only as a Monte Carlo benchmark when the caps make it computationally feasible. No cross-validation is used to choose the nuclear penalty because the penalized estimator is only a screening and warm-start device; final rank selection is based on unpenalized post-refits and the information criterion.

Rank stability is assessed using quantities already generated by the procedure. The implementation must report: the gap between the smallest and second-smallest information criteria; whether the selected rank changes on the denser path; whether it changes under the threshold and penalty multipliers; whether any selected rank equals its cap; and how the main plug-in estimates change under the best one-coordinate neighboring ranks. When a neighboring rank has a nearly identical criterion, the corresponding point-estimate sensitivity is reported even though the primary theorem continues to condition on consistent rank selection. These checks do not create rank-robust inference; they reveal when the exact-rank approximation is empirically fragile.
...
A practical implementation proceeds as follows.
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
\item \textit{Preprocessing and caps.} Store canonical units, compute the block RMS weights, and specify each reporting cap $\bar r_M$, its pilot cap $\bar r_M+1$, and the unchanged coefficient box.
\item \textit{Optional initialization.} If desired, compute a deterministic nuclear-norm path only to provide starting singular spaces for the two constrained least-squares fits. No path rank is a statistical candidate or selected rank.
\item \textit{Joint spectral pilot.} Compute the one cap+1 pilot in \eqref{eq:rank_cap_pilot} and record feasibility, all start objectives and stationarity residuals, objective stability, box activity, numerical ranks, and termination status.
\item \textit{Spectra.} Extract singular values through $\bar r_M+1$ for every block and construct \eqref{eq:rank_normalized_spectrum}.
\item \textit{Ridge ratios.} Set $a_{NT}=1/\log(NT)$, compute \eqref{eq:rank_zero_ratio} and \eqref{eq:rank_ratio}, and select each block rank by \eqref{eq:rank_selector}.
\item \textit{Final post-refit.} Compute one literal unpenalized constrained fixed-rank joint fit $\widehat\bTheta(\widehat\br)$. The pilot is not substituted for this fit.
\item \textit{Inference.} From the final post-refit, construct the tangent projections, empirical Riesz equations, plug-in targets, four fixed-rank split fits where required, two-way correction, residuals, spatial variances, and intervals.
\end{enumerate}

If the cap+1 pilot is infeasible, nonfinite, fails the prespecified objective/stationarity tolerances, or remains unstable across the maintained starts, record \texttt{rank\_selection\_numerically\_unresolved}. There is no fallback information-criterion rank, thresholded pilot rank, nuclear-path rank, or supplied-truth rank, and no primary selected-rank point estimate or inference is reported. This rule is separate from a boundary-active final post-refit, whose point-estimate and interiority treatment remains as stated below.

Each accepted cap+1 pilot and final post-refit uses the maintained multistart objective-stability and first-order-residual checks. These are observable numerical diagnostics, not certificates of the theoretical global objective gap. The selected full-panel post-refit supplies starting values for all split-panel fits. Matrix-free tangent projection and Riesz solution, neighbor-list spatial variance computation, and storage of the balanced split seeds remain unchanged.

\subsection{Tuning choices}
\label{subsec:tuning}

The rank selector has only the prespecified reporting caps, pilot caps equal to reporting caps plus one, and the deterministic ridge $a_{NT}=1/\log(NT)$. It has no information-criterion multiplier, penalty sensitivity, ridge multiplier, rank-zero cutoff, or singular-value threshold. The caps and rule are fixed before results are examined.

If the optional nuclear path is used for warm starts, the existing geometric grid with $\gamma=0.8$ and endpoint ratio approximately $0.01$ may be retained solely as a computational initialization grid. Its resolution, thresholded ranks, and endpoint do not define the mathematical selector or enter \Cref{thm:rank_consistency}. Optimization tolerances are prespecified numerical choices and are reported with the pilot and final post-refit diagnostics.

The coefficient bound $B$ is fixed ex ante from a deterministic support bound for the maintained coefficient matrices. The margin $c_B$ in \Cref{a:signal} is a theoretical interior margin and is not selected from the reported samples. In the Monte Carlo design, the final coefficient envelope is verified to satisfy $\|\bTheta_0\|_{\max}\le B-c_B$ uniformly over the maintained cells. A boundary-active constrained solution remains a valid point estimate of the finite-sample constrained problem, but it is recorded as a failed interiority diagnostic because the asymptotic inference argument uses the event that the coefficient bound is inactive.

For general spatial dependence, use a prespecified positive-semidefinite spatial weight matrix and a distance cutoff $h_N=\lceil c_h\log(NT)\rceil$. Results should be reported over a prespecified range of $c_h$. Conditional cross-sectional independence uses $\bm W_N=\bm I_N$, while known clusters use within-cluster weights.

Optimization tolerances are stated relative to the scale of the objective and the estimated standard error. The implementation reports the relative objective change, the fixed-rank first-order residual, and the Riesz-equation residual. The random balanced time and unit partitions and their seeds are stored. The primary interval is not reported when rank selection is numerically unresolved, no final or split numerical solution satisfies the tolerances, the coefficient bound is active, a Riesz system or split-panel Gram matrix is nearly singular, or the chosen spatial variance calculation fails its positive-semidefiniteness check.
...
For the Monte Carlo ridge-ratio selector, the reporting rank caps are
$$
(\bar r_A,\bar r_B,\bar r_H)=(3,3,3)
$$
for both the main rank-one designs and the separate rank-stress designs. The cap+1 spectral-pilot caps are therefore $(4,4,4)$ and are not reportable. In every replication, one joint spectral pilot produces the normalized block spectra and ridge ratios, each block rank is selected separately, and one final fixed-rank post-refit is computed at the selected vector. There is no ridge multiplier or sensitivity experiment. These rules are fixed before the reported replications are generated and contain every maintained true rank vector.

For rank reporting, ``Exact'' means $\widehat\br=\br_0$. ``Under only'' means every coordinate is weakly below truth and at least one is strictly below; ``Over only'' is defined symmetrically; and ``Mixed'' requires at least one underselected and one overselected coordinate. A failed pilot acceptance check is reported separately as numerically unresolved. The rank table also reports pilot validity, cap-selected counts, and the modal selected vector. Balanced $N=T$ cells are baseline design choices. The rectangular cells retain unequal $N,T$ to assess the asymmetric growth allowed by \Cref{a:growth}.
...
\input{tables/mc/mc_rank_revision10}
...
The production figures are prespecified before the final replications are examined. Figure~\ref{fig:mc_bias_rmse} shows convergence of bias and RMSE; Figure~\ref{fig:mc_coverage} shows coverage and standard-error accuracy; Figure~\ref{fig:mc_rank_recovery} shows ridge-ratio exact, under, over, and mixed recovery by block and panel size together with ratio-separation diagnostics; Figure~\ref{fig:mc_power} reports DGP~4 power; and Figure~\ref{fig:mc_retention} reports retention and numerical-failure rates. No figure is devoted to an obsolete information-criterion gap. All Monte Carlo uncertainty bands use the relevant denominator for the statistic being plotted.

For replication $r$, let
\[
e_r=\widehat\phi_r-\phi_r
\]
denote the estimation error, and let $\mathcal P$ and $\mathcal I$ denote
the sets of point-valid and inference-valid replications, respectively.
The reported bias and RMSE are the mean and root mean square of $e_r$
over $\mathcal P$. Because the exact simulated truth may vary across
replications, ``MC SD'' is the standard deviation of $e_r$, rather than
of the raw estimates $\widehat\phi_r$. ``Mean SE'' is the average
estimated standard error over $\mathcal I$, while ``SE/SD'' compares
that average with the standard deviation of $e_r$ computed over the
same inference-valid sample $\mathcal I$. ``Cov.'' is empirical
95\% coverage, and ``Rej.'' is the rejection probability of the
two-sided test of $H_0:\phi=0$. ``Point/Inf. ret.'' reports
$R_{\mathrm{point}}/R_{\mathrm{attempted}}$ and
$R_{\mathrm{inference}}/R_{\mathrm{attempted}}$, respectively.
Finite estimates are never removed because of their magnitude.
...
This paper develops full-panel estimation and inference for dynamic panels with several separately ranked coefficient matrices. A joint cap+1 spectral pilot and fitted-value-normalized ridge ratios select each block rank, while all reported coefficients are obtained from one unpenalized joint fixed-rank post-refit. The analysis aligns and expands the estimated singular spaces and uses a target-specific Riesz representer to translate estimation errors in the autoregressive, slope, and interactive-effect matrices into first-order uncertainty for a selected parameter. The framework allows predetermined covariates, conditioning on common shocks, dynamic feedback through lagged outcomes, and spatially dependent unit-specific innovations.

The inferential procedure depends on the parameter being reported. Individual coefficients, fixed unit--time lag sums, and suitable fixed-time means and contrasts use one full-panel fit because the leading $T^{-1}$ loading bias and $N^{-1}$ factor bias are smaller than their standard errors. Averages whose weights extend over many units and many time periods receive a fixed two-way split-panel correction that removes both leading bias terms. Rank selection is performed once on the full panel from the normalized cap+1 pilot spectra. The rank-consistency argument depends on the pilot operator rate, strong singular values, and deterministic ridge separation under rectangular growth. A nuclear path, if used, serves only as numerical initialization.

The scope of the theory is deliberately narrower than that of some single-matrix low-rank models. The inferential results assume exact fixed ranks and consistent selection of the separate rank vector; the confidence intervals are not presented as approximate-low-rank or rank-misspecification-robust procedures. Allowing growing ranks would change the dimensions of the loading and factor regressions, the uniform concentration rates, and the split-panel bias calculation. Rank-robust inference would require correcting the effects of overestimating several interacting latent spaces, including their propagation through the dynamic equation. Finally, the present Riesz equation establishes valid inverse-Gram adjustment under conditional heteroskedasticity and spatial dependence but does not characterize the semiparametric efficiency bound; an efficient extension would require covariance-optimal weighting of the joint Gram equation. These questions are natural directions for future work rather than consequences of the current results.

The Monte Carlo and empirical sections are designed to evaluate the finite-sample bias correction, spatial inference, rank-selection stability, and computational gains of the proposed full-panel procedure. Their numerical tables and figures will be populated after implementation is complete.
...
\bibitem[Barigozzi et~al.(2023)]{barigozzihe2023}
Barigozzi, M., He, Y., Li, L., and Trapani, L. (2023).
Robust tensor factor analysis.
\emph{arXiv preprint arXiv:2303.18163}.
...
\bibitem[Gao and Absil(2022)]{gaoabsil2022}
Gao, B., and Absil, P.-A. (2022).
A Riemannian rank-adaptive method for low-rank matrix completion.
\emph{Computational Optimization and Applications}, 81, 67--90.
\doilink{10.1007/s10589-021-00328-w}.
...
\bibitem[Moon and Weidner(2026)]{moonweidner2026}
Moon, H.~R., and Weidner, M. (2026).
Nuclear norm regularized estimation of panel regression models.
\emph{Journal of Econometrics}, 255, Article 106246.
\doilink{10.1016/j.jeconom.2026.106246}.
...
\right].
$$
Because $\Gamma_0$ is coercive on $\calT_0$, the Riesz equation has a unique solution. Taking $\bDelta=\bq_0$ in \eqref{eq:population_target_weight} gives
\begin{equation}
...
\end{pmatrix}.
$$
The diagonal block associated with loading row $i$ is exactly the conditional time-series Gram matrix in \Cref{a:gram}; the diagonal block associated with factor row $t$ is the corresponding conditional cross-sectional Gram matrix. Hence these diagonal blocks have eigenvalues between $c$ and $C$. The local part of \Cref{a:identification} makes the full operator coercive, and therefore bounds both Schur complements away from zero.
...
\]
Nuclear/operator-norm duality and the fixed rank caps now give, uniformly over $\calD_{\max}$,
\[
\left|
\sum_{i,t}u_{it}\mathcal Y_{it}(\bDelta)
\right|
...
\right\}
$$
for every split tangent direction $\bDelta$. Taking $\bDelta=\bm d_{T,h}$ and using split Gram coercivity yields
$$
...
The rank theorem separates candidate coverage from final model comparison. Candidate coverage depends only on whether the candidate set contains the thresholded rank vector of one sufficiently accurate pilot. The nuclear path is useful for screening and warm starts, but no particular path value or grid resolution is used in the consistency argument. Conditional on candidate coverage, the post-refit information criterion rejects underfitted and overfitted rank vectors.

\begin{proposition}[Pilot thresholding and candidate coverage]
\label{prop:rank_candidate_coverage}
Suppose the candidate set contains the thresholded rank vector of a pilot $\widehat\bTheta^{\mathrm{pil}}$ satisfying
$$
\max_M
\|\widehat\bM^{\mathrm{pil}}-\bM_0\|_{\op}
=
...
Then
$$
\Pzero(\br_0\in\calR_{\mathrm{cand}}\mid\mathscr C)
\longrightarrow_p1.
$$
Under the maintained assumptions, the rank-cap pilot in \eqref{eq:rank_cap_pilot} satisfies
$$
\max_M
\|\widehat\bM^{\max}-\bM_0\|_{\op}
=
O_p\{\sqrt{NT\zeta_{NT}}\}
$$
when its normalized objective gap is $o_p(\zeta_{NT})$. Hence it supplies candidate coverage whenever
$$
\sqrt{NT\zeta_{NT}}=o(\tau_{NT})
\quad\text{and}\quad
\tau_{NT}=o(\sqrt{NT}).
...
\begin{proof}
For every coefficient matrix, Weyl's singular-value perturbation inequality gives
$$
\left|
\sigma_s(\widehat\bM^{\mathrm{pil}})
-
\sigma_s(\bM_0)
\right|
\le
\|\widehat\bM^{\mathrm{pil}}-\bM_0\|_{\op}.
$$
If $s>r_{M,0}$, then $\sigma_s(\bM_0)=0$, so the corresponding estimated singular value is $o_p(\tau_{NT})$. If $s\le r_{M,0}$, \Cref{a:signal} gives $\sigma_s(\bM_0)\ge c\sqrt{NT}$, whereas $\tau_{NT}=o(\sqrt{NT})$ and the pilot error is $o_p(\tau_{NT})$. Thus, thresholding the pilot at $\tau_{NT}$ recovers $r_{M,0}$ for every coefficient matrix with conditional probability approaching one. Since the thresholded pilot rank is included in $\calR_{\mathrm{cand}}$, candidate coverage follows.

It remains to verify the stated rate for the baseline rank-cap pilot. The truth is feasible for \eqref{eq:rank_cap_pilot}. Let
$$
\widehat\bDelta^{\max}
=
\widehat\bTheta^{\max}-\bTheta_0.
$$
Every matrix difference has rank at most $\bar r_M+r_{M,0}$. The same least-squares basic inequality, score bound, and conditional prediction lower bound used in \Cref{thm:recovery} therefore give
$$
\|\widehat\bDelta^{\max}\|
=
O_p\{\sqrt{NT\zeta_{NT}}\}.
...
O_p\{\sqrt{NT\zeta_{NT}}\}.
$$
The final threshold conditions then imply the generic pilot requirement.
\end{proof}

The same perturbation argument also proves the screening statement in \Cref{rem:approx_rank_screening}; no additional approximate-low-rank inference claim is used in the rank-consistency proof.

\begin{proof}[Proof of \Cref{thm:rank_consistency}]
By \Cref{prop:rank_candidate_coverage}, it is enough to compare the incorrect vectors in $\calR_{\mathrm{cand}}$ with $\br_0$ on the event $\{\br_0\in\calR_{\mathrm{cand}}\}$.

If $\br$ is underfitted, at least one fitted coefficient matrix omits a true singular direction of order $\sqrt{NT}$. Hence
$$
\inf_{\bTheta\in\calM(\br)}
\|\bTheta-\bTheta_0\|^2
\ge
cNT.
$$
Conditional prediction identification converts this into a residual-loss increase bounded away from zero after division by $NT$. The largest possible reduction in the dimension penalty is of order
$$
\kappa_{NT}\frac{N+T}{NT}
=
o(1)
$$
by \eqref{eq:rank_penalty_rates}. Therefore, an underfitted candidate cannot minimize \eqref{eq:rank_selector}.

If $\br$ is overfitted, the least-squares basic inequality and \Cref{prop:uniform_concentration} show that its best normalized residual improvement relative to $\br_0$ is $O_p(\zeta_{NT})$. Adding one rank to one $N\times T$ coefficient matrix raises $d(\br)$ by order $N+T$, so the penalty increase is of order
$$
\kappa_{NT}\frac{N+T}{NT},
$$
which dominates $\zeta_{NT}$ by \eqref{eq:rank_penalty_rates}. The rank caps and number of coefficient matrices are fixed, so $\calR_{\max}$, and hence $\calR_{\mathrm{cand}}$, contains only finitely many distinct vectors. Numerical objective errors are smaller than the same separation by assumption. Therefore,
$$
\Pzero(\widehat\br=\br_0\mid\mathscr C)
\to_p1.
$$
On this event, the selected full- and split-panel estimators use the same rank restrictions as the supplied-rank estimators, which transfers all supplied-rank limits.
\end{proof}


\fi

\begin{proof}[Proof of \Cref{cor:cap_plus_one_pilot}]
For $\bDelta\in\calD_{\max}^{+}$, rank subadditivity gives
$\rank(\bDelta_M)\le\bar r_M+1+r_{M,0}\le2\bar r_M+1$. These bounds and the number of blocks are fixed. Nuclear/operator-norm duality therefore extends \eqref{eq:uniform_score_bound} to $\calD_{\max}^{+}$ with only the fixed multiplier $\{\sum_M(\bar r_M+1+r_{M,0})\}^{1/2}$. Likewise, the singular-vector covering entropy in the proof of \Cref{prop:uniform_concentration} remains bounded by $C_+(N+T)\log(Cb_{NT})$, where $C_+$ depends only on the fixed enlarged ranks. The truncation, blocking, peeling, and Lipschitz steps and all $N,T$ rates are unchanged. Using \Cref{a:pilot_identification}, uniformly on $\calD_{\max}^{+}$,
$$
\left|\sum_{i,t}u_{it}\mathcal Y_{it}(\bDelta)\right|
\le O_p\{\sqrt{NT\zeta_{NT}}\}\|\bDelta\|,
$$
and
$$
\sum_{i,t}\mathcal Y_{it}(\bDelta)^2
...
Let $n=NT$, $\widehat\bDelta^{\mathrm{pil}}=\widehat\bTheta^{\mathrm{pil}}-\bTheta_0$, and let $\delta_{NT}$ denote the left side of \eqref{eq:pilot_objective_gap}. Truth is feasible, so the exact loss expansion and approximate optimality give
$$
...
+n\delta_{NT}.
$$
Substitution of the preceding bounds yields
$$
\frac{c_+}{4}\|\widehat\bDelta^{\mathrm{pil}}\|^2
\le
O_p\{\sqrt{n\zeta_{NT}}\}\|\widehat\bDelta^{\mathrm{pil}}\|
+O_p(n\zeta_{NT})+n\delta_{NT}.
$$
Since $\delta_{NT}=o_p(\zeta_{NT})$, solving the quadratic gives
$\|\widehat\bDelta^{\mathrm{pil}}\|=O_p\{\sqrt{NT\zeta_{NT}}\}$. Each block operator norm is bounded by the joint Frobenius norm, proving the corollary.
\end{proof}

\begin{proof}[Proof of \Cref{thm:rank_consistency}]
Write $q_M=w_M/w_{A,1}$. The maintained moment, innovation, stability, and prediction-identification conditions imply that the finite collection $\{q_M\}$ is bounded above and away from zero with probability approaching one. Weyl's inequality and \eqref{eq:pilot_operator_rate} give, jointly over fixed blocks and indices,
$$
\widehat\lambda_{M,j}=\Theta_p(1)
\quad (j\le r_{M,0}),
\qquad
\widehat\lambda_{M,j}=O_p(\zeta_{NT})
\quad (j>r_{M,0}).
$$
For positive singular values the lower bound follows from \Cref{a:signal}; after truth, $\sigma_j(\bM_0)=0$ and the squared operator perturbation divided by $NT$ is $O_p(\zeta_{NT})$.

Let $a_{NT}=1/\log(NT)$. The rectangular identity following \Cref{cor:cap_plus_one_pilot} gives $a_{NT}\to0$ and $\zeta_{NT}/a_{NT}\to0$. Fix a block with $0<r=r_{M,0}\le\bar r_M$. Before truth, the adjacent signal values are bounded above and away from zero, so every $R_{M,j}$, $1\le j<r$, is bounded away from zero; the anchored ratio $R_{M,0}$ is also bounded away from zero. At truth,
$$
R_{M,r}
=
...
\longrightarrow_p0.
$$
After truth, both spectral values are $o_p(a_{NT})$, so $R_{M,j}\to_p1$. When $r=\bar r_M$, the numerator at truth uses the permitted $(\bar r_M+1)$st pilot singular value and is $O_p(\zeta_{NT})$ rather than being set to zero by the reporting cap.

If $r_{M,0}=0$, all pilot spectral values are $O_p(\zeta_{NT})$, whence
$$
R_{M,0}
=
\frac{a_{NT}+o_p(a_{NT})}{1+a_{NT}}
\longrightarrow_p0,
\qquad
R_{M,j}\longrightarrow_p1
\quad (j\ge1).
$$
Thus the true rank is the unique asymptotic blockwise minimizer, including for the zero block in $(1,0,2)$. Since the number of coefficient matrices is fixed,
$$
...
\longrightarrow_p0.
$$
On $\{\widehat\br=\br_0\}$, the final selected-rank post-refit is the supplied-rank estimator. The recovery, Riesz, target-expansion, split-correction, spatial-variance, and feasible-normality conclusions therefore transfer by asymptotic equivalence.
\end{proof}
...
\subsection{Ridge-ratio rank, spatial-weight, and computation diagnostics}

The ridge-ratio appendix reports, by coefficient block, the true and selected ranks, normalized pilot singular values, all reportable ratios, the smallest and second-smallest ratios and their gap, pilot feasibility and boundary activity, multistart objective diagnostics, and final post-refit validity. Candidate counts, information criteria, penalty or threshold multipliers, and local completion are not Revision-10 diagnostics.

\input{tables/mc/app_mc_ridge_ratio_diagnostics}
...
\placeholdertable{Empirical ridge-ratio rank selection}{tab:app_emp_rank}{Placeholder for reporting and cap+1 pilot caps, normalized pilot singular values, blockwise ridge ratios, selected ranks, smallest and second-smallest ratios, ratio margins, pilot feasibility, multistart objective and stationarity diagnostics, boundary activity, and final post-refit validity. Neighboring supplied-rank estimates may be reported separately as sensitivity diagnostics but do not define the selected rank.}
